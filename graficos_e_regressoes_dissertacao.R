## READ.ME:
#Nesse script serão realizados os filtros, criação de gráficos, tabelas e regressões que serão utilizadas na análise
...
dir() #vendo o que tem no diretório, para conferir se a base está lá 

df_scr<- data.table::fread("df_scr_final.csv") #base utilizada na presente análise

### Filtros e variáveis criadas
{
## como a análise é para empresas, não faz sentido termos pessoa física na base, então iremos retira-las da nossa base

df_scr_sem_pf<- df_scr%>%
  filter(cliente !="PF")
#verificando se apenas os clientes pessoa jurídica estão na base

table(df_scr$cliente,exclude = NULL)
# certo, temos apenas pessoa jurídicam vamos prosseguir
...
#Agora criaremos as primeira dummies, sendo elas para identificar a modalidade da empresa, porte, tipo de origem e etc,pois trabalhar com variáveis str é
#muito suscetível ao erro

### dummies para o mercado de crédito####
...
#verificando se está correta a distribuição
...
#tirando as pjs indisponíveis, pois elas não interessam apra a análise 
...
{
  ##### PARTE INICIAL, CRIAÇÃO DE BASES E FILTROS ####
...
#  divididos pelo describeBy
  ...
  df_carteira_ativa_total<- df_scr_sem_pf %>% #OBS APENAS APÓS MARCO#
 ...
 carteira_ativa_total_describe = describeBy(df_carteira_ativa_total$carteira_ativa_bilhao, group = df_carteira_ativa_total$crise_marco)
  ...
  #### AGORA AS TABELAS DAS ESTATÍSTICAS ####
## aqui está o que realmente importa!!!
  
  ## Tabela 1 - Estatísticas descritivas antes e após a crise do COVID-19 dos subgrupos 
  #carteira_ativa
  
  df_carteira_ativa_subgrupos<- df_scr_sem_pf %>% #OBS APENAS APÓS MARCO#
...
## REGRESSÕES ###
{
  ...
  ### tabela das regressões
  
  ## Tabela 2 - Efeito da pandemia no mercado de crédito- Efeitos heterogêneos
